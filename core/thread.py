...
class Thread:
    ...
    async def setup(self, *, creator=None, category=None):
        ...
        footer = "Votre message a été envoyé"
    ...
    async def _restart_close_timer(self):
        ...
        close_message = self.bot.config.get(
            ...
            f"Ce ticket s'est fermé automatique pour cause dinactivité "
            f"after {human_time}.",
        )
        time_marker_regex = "%t"
    ...
    async def send(
        ...
        if from_mod:
            ...
            embed.set_footer(text=f"Destinataire")
            # noinspection PyUnresolvedReferences,PyDunderSlots
...
class ThreadManager:
    ...
    def format_info_embed(self, user, log_url, log_count, color):
        ...
        if user not in self.bot.guild.members or len(mutual_guilds) > 1:
            embed.add_field(
                name="Serveurs en commun :", value=", ".join(g.name for g in mutual_guilds)
            )
